...
def _build_volume_2d_psmnet( refimg_fea, targetimg_fea, maxdisp):
    cost = Variable(
        torch.FloatTensor(refimg_fea.size()[0], refimg_fea.size()[1] * 2, maxdisp // 4, refimg_fea.size()[2],
...
def warp(x, disp):
    ...
    #utput = nn.functional.grid_sample(x, vgrid, align_corners=True )
...
def _build_cost_volume(cost_volume_type, refimg_fea, targetimg_fea, maxdisp):
    if cost_volume_type == "Concat":
        ...
        cost = _build_volume_2d_psmnet(refimg_fea, targetimg_fea, maxdisp=maxdisp)


    elif cost_volume_type == "Distance_based":

        cost = _build_volume_2d_anynet(refimg_fea, targetimg_fea, maxdisp // 4, stride=1)
        cost = torch.unsqueeze(cost, 1)


    elif cost_volume_type == "Difference":
        #print("build difference")
        cost = _build_volume_2d_difference(refimg_fea, targetimg_fea, maxdisp // 4)
        #print("cost size:", cost.shape)
